...
const RESOURCE_MAP = {
  ...
  "news-events": {
    table: "news_events",
    writable: ["item_type", "title", "summary", "content", "image_url", "video_url", "event_date", "published"]
  },
  gallery: {
    table: "gallery_items",
...
async function ensureNewsSchema(pool) {
  const exists = await tableExists(pool, "news_events");
  if (!exists) {
    await pool.query(`
    ...
    const cols = new Set(await columnsFor(pool, "news_events"));
    const additions = {
      item_type: "TEXT",
      title: "TEXT",
      summary: "TEXT",
    ...
    for (const [name, type] of Object.entries(additions)) {
      if (!cols.has(name)) {
        await pool.query(`ALTER TABLE public.news_events ADD COLUMN ${quoteIdent(name)} ${type}`);
      }
    }
  }
}
...
async function getConfig(pool, resource) {
  ...
  if (resource === "news-events") await ensureNewsSchema(pool);
